...
# versions without rerunning the job. The same script also runs from the
# workflow just before `Execute CI task` to catch state drift between
# install and execute. Best-effort: never fail install if a probe is
# missing.
bash "${SCRIPT_DIR}/dump_env_info.sh" "post-install" || true
