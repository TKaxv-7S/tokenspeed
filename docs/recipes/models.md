...
the shared padding-preserving workspace-to-slot mapping stays capture-friendly
without importing the DSA backend into generic drafter startup. The draft MoE
path is also excluded from expert-distribution recording. The GLM DSA decode
top-k schedule cache is refreshed against the current MTP query width and
visible lengths.
